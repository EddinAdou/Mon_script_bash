...
if [ "$#" -lt 1 ] || ([ "$1" != "user" ] && [  "$1" != "install" ] && [ "$1" != "configure_site" ] && [ "$1" != "active_site" ]); then
        echo "Usage: $0 user <username> <password> or $0 install or $0 configure_site <site_name> <http_port> or $0 active_site <site_name>"
        exit 1
fi
